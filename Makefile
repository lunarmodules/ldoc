...
RUN=&& lua $(_REPODIR)/ldoc.lua . && diff -r out cdocs && echo ok

test-prep:
	find -type d -name out -execdir rsync -av --del {}/ cdocs/ \;
...
CLEAN=&& lua $(_REPODIR)/ldoc.lua . && rd /S /Q cdocs && cp -rf out cdocs
